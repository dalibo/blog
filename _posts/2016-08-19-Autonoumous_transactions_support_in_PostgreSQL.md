...
It's interesting to compare performances between this two extensions,
...
give you an idea of what performances gain can be expected.
...
performances are quite the same up to 10 parallel clients. The *pg_background*
...
of my computer with *pg_background* so I just used up to 8 parallel clients.
...
In asynchronous mode, *pg_background* is obviously most performing than *dblink*
but you quickly reach the limit of the dynamic shared memory allocation. Errors
start to appears with just 6 parallel clients:
```
...
For the asynchronous mode here are the function used with *dblink*:
